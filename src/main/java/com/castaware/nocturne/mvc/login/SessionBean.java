...
import com.castaware.nocturne.NocturneModule;
...
import eic.tcc.control.menu.MenuCategory;
import eic.tcc.control.menu.MenuItem;
...
{             
	...
	public SessionBean()
	{
		...
		username="admin";
		roles.add("ADMIN");
		rolesMap.put("ADMIN",new LoggedRole("ADMIN",null));
		modules.add(new NocturneModule(false));
	 ...
	 */
	void setMenu(Menu scaMenu) 
	{
		this.menu = new DefaultMenuModel();
		
		for (MenuCategory scaCategory : scaMenu.getCategories())
		{
			DefaultSubMenu menuElement = DefaultSubMenu.builder()
													   .label(scaCategory.getName())
													   .build();
			
			menu.getElements().add(menuElement);
			
			for (MenuItem scaItem : scaCategory.getItems())
			{
				DefaultMenuItem menuItem = DefaultMenuItem.builder()
														  .value(scaItem.getName())
														  .icon(null)
														  .url("."+scaItem.getUrl()+"?refresh=true")
														  .build();
				
				menuElement.getElements().add(menuItem);
			}
		}		
	}
